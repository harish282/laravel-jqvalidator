(function ($) {
    ...
    var namespace = '.validator',
            ...
            validations = {
                ...
                validate: function (options) {
                    ...
                    $(selector).each(function () {
                        ...
                        $.each(rules, function (idx, rule) {
                            ...
                            thisValid = method ?
                                    method.apply(validations, [name, value, rule.params, self]) :
                                    false;
                ...
                implicit: function (rule) {
                    ...
                    return rule.method === "validate_required" ||
                            ...
                            rule.method === "validate_required_without" ||
                            rule.method === "validate_required_without_all" ||
                ...
                validate_required_without: function (attribute, value, parameters) {
                    
                    var all_other_present = true;
                    for(var i in parameters) {
                        if(!this.validate_required(parameters[i], $("[name=\"" + parameters[i] + "\"]").val())) {
                            all_other_present = false;
                        }
                    }
                    
                    if(all_other_present) return true;
                    
                    return this.validate_required(attribute, value);
                },
                validate_required_without_all: function (attribute, value, parameters) {
                    for(var i in parameters) {
                        if(this.validate_required(parameters[i], $("[name=\"" + parameters[i] + "\"]").val())) return true;
                    }
                    return this.validate_required(attribute, value);
                },
                ...
                validate_mimes: function (attribute, value, parameters) {
                 ...
                 validate_nullable: function (attribute, value, parameters) {
                    return true;
                    return value == null || typeof value == 'undefined';
                },
                 validate_date: function (attribute, value, parameters, ele) {
                     if(moment) {
                         return moment(value, $(ele).data('date-format')).isValid();
                     }
                     return true;
                    //return (Date.parse(value) );
                },
                validate_before: function (attribute, value, parameters, ele) {
                    var otherVal = $(ele).closest('form').find("[name="+parameters[0]+"]").val();
                    if (moment) {
                        return moment(value, $(ele).data('date-format')).isBefore(moment(otherVal, $(ele).data('date-format')));
                    }  else{  
                        return (Date.parse(value) < Date.parse(parameters[0]));
                    }

                },
                validate_after: function (attribute, value, parameters, ele) {

                    if (moment) {
                        var otherVal = $(ele).closest('form').find("[name="+parameters[0]+"]").val();
                        return moment(value, $(ele).data('date-format')).isAfter(moment(otherVal, $(ele).data('date-format')));
                    }  else{  
                        return (Date.parse(value) > Date.parse(parameters[0]));
                    }
            ...
            methods = {
                ...
                makebind: function (options) {
                    ...
                    $(document).on(options.events, options.bind, function (evt) {
                        var valid;
    ...
    $.fn.bindValidator = function (method) {

        if (methods[method]) {
            return methods[method].apply(this, Array.prototype.slice.call(arguments, 1));
        } else if (typeof method === 'object' || !method) {
            return methods.makebind.apply(this, arguments);
        } else {
            $.error('Method ' + method + ' does not exist on jQuery' + namespace);
        }
    ...
    $.fn.formValidator = function () {
        $(this).bindValidator({
            ...
            callback: function (elem, valid) {
                if (!valid) {
                    $(elem).parents('.input-group,.form-group').removeClass('has-success').addClass('has-error');
                    $(elem).removeClass('has-success').addClass('has-error');
                } else {
                    $(elem).parents('.input-group,.form-group').removeClass('has-error').addClass('has-success');
        ...
        $(this).find("input[data-validations*='required'],textarea[data-validations*='required'],select[data-validations*='required']").each(function () {
            $(this).parent().find('label').append("<small class='text-red'>*</small>");
            var pval = $(this).attr('placeholder');
            if(pval && pval !='' && pval.indexOf('*') == -1) {
    ...
    $(function () {
        ...
        $('.jqform').find("input[data-validations*='required'],textarea[data-validations*='required'],select[data-validations*='required']").each(function () {
            $(this).closest('.input-group,.form-group').find('label').append("<small class='text-red'>*</small>");
            var pval = $(this).attr('placeholder');
            if(pval && pval !='' && pval.indexOf('*') == -1) {
                $(this).attr('placeholder', pval+" *");
            }
